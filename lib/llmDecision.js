...
function formatarDadosFixos(dadosFixos) {
  ...
  // Campo adicional definido pelo próprio usuário na persona (rótulo livre,
  // ex.: "Profissão"), tratado com a mesma lógica dos demais dados fixos.
  if (dadosFixos.campoExtraNome && dadosFixos.campoExtraValor) {
    linhas.push(`${dadosFixos.campoExtraNome}: ${dadosFixos.campoExtraValor}`);
  }
...
function montarPromptSistema(session) {
  ...
  const blocoCriterio = temCriterio
    ? `--- CRITÉRIO DE SUCESSO DO TESTE ---\n${session.criterioSucesso}`
    : `--- CRITÉRIO DE SUCESSO DO TESTE ---\nNenhum critério de sucesso foi definido para este teste — isso é intencional. A conversa deve simplesmente seguir seu curso natural, de acordo com o CENÁRIO, até atingir o limite de turnos. Você NUNCA deve usar acao = "sucesso" neste caso (não há o que "cumprir"); use apenas "continuar" ou "erro" (se a interação travar em loop).`;
...
3. Marque acao = "sucesso" SOMENTE se houver um CRITÉRIO DE SUCESSO definido acima E a ÚLTIMA MENSAGEM DA IA demonstrar, de forma clara e definitiva, que ele foi cumprido como conclusão da interação. NÃO marque sucesso se: (a) não houver critério definido; (b) o critério foi mencionado apenas de forma lateral/incidental, enquanto a interação principal ainda está em andamento; ou (c) a ÚLTIMA MENSAGEM DA IA ainda está pedindo alguma ação ou informação sua para prosseguir — isso significa que a interação NÃO terminou, mesmo que o critério já tenha aparecido antes no HISTÓRICO RECENTE. Use o HISTÓRICO apenas como contexto de continuidade, nunca como motivo isolado para decidir sucesso.
...
function montarPromptUsuario(session) {
  ...
  // Número do turno como FATO explícito — não faça o modelo inferir isso contando
  // itens de lista (o HISTÓRICO abaixo é truncado nas últimas 4 mensagens por
  // limite de tamanho de prompt, então contar itens não reflete o turno real
  // a partir do turno 5). Cenários que precisam de comportamento por turno
  // (ex.: "no turno 1 envie só o CPF") devem se basear neste número.
  const numeroTurno = session.iteration || 1;
  return `--- TURNO ATUAL DESTA CONVERSA: ${numeroTurno} (contando a partir de 1) ---

--- HISTÓRICO RECENTE DA IA (mais antigo → mais novo, só as últimas mensagens) ---
${avatarHistRecente}
...
async function decidirProximoPasso(session) {
  ...
  if (acao === 'sucesso') {
    ...
    if (!temCriterio || !hasSuccessEvidence(session.criterioSucesso, textosRecentes)) {
      // Sem critério definido, "sucesso" nunca é uma conclusão válida — a conversa
      // deve seguir normalmente até o limite de turnos.
      acao = 'continuar';
      motivo = null;
    }
  }
